  ...
  const columns: Column<Subject>[] = [
    ...
    { header: 'Jenis', accessor: (row) => (
      <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
        row.jenis === 'UMUM' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800'
      }`}>
        {row.jenis}
      </span>
    ), width: '120px' },
    ...
    { header: 'Aksi', accessor: (row) => (
      <div className="flex gap-2">
        ...
        <button onClick={() => openDelete(row)} className="p-1 text-red-600 hover:bg-red-50 rounded">
          <Trash2 size={16} />
        </button>
      </div>
    ), width: '100px' },
