...
function TAV() {
  ...
  return(<div className="space-y-6">
    ...
    {selC&&(<div className="bg-slate-800 rounded-xl shadow-lg shadow-black/20 border border-slate-700 p-6"><div className="flex items-center gap-2 mb-4"><div className="w-8 h-8 bg-primary-900/40 rounded-lg flex items-center justify-center"><span className="text-primary-300 font-bold text-sm">2</span></div><h3 className="text-lg font-semibold text-slate-100">Pilih Tanggal</h3></div><div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4"><input type="date" value={selD} onChange={e=>{var d=new Date(e.target.value);var jsDay=d.getDay();var javaDay=jsDay===0?7:jsDay;if(sS&&sS.day_of_week!=null&&sS.day_of_week!==javaDay){alert("Hari ini bukan hari mengajar! Jadwal mengajar hari "+DN[sS.day_of_week||0]);return}setSelD(e.target.value);setSd(false)}} className="bg-slate-800 text-slate-100 border-2 border-slate-700 rounded-xl px-4 py-3 font-medium focus:border-primary-500 focus:outline-none"/>{sS&&<div className="bg-primary-900/30 text-primary-300 px-4 py-3 rounded-xl"><span className="font-medium">{sS.subject_name}</span><span className="text-sm ml-2">({sS.class_name})</span></div>}</div></div>)}
    {selC&&ent.length>0&&(<div className="bg-slate-800 rounded-xl shadow-lg shadow-black/20 border border-slate-700 p-6"><div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4"><h3 className="text-base sm:text-lg font-semibold text-slate-100">Daftar Siswa - {sS?.class_name}</h3><div className="flex items-center gap-2"><span className="text-xs sm:text-sm text-slate-400">Set semua:</span>{SO.map(s=><button key={s.value} onClick={()=>sa(s.value)} className={"px-3 py-1.5 rounded-lg text-xs font-medium "+s.bgColor+" "+s.textColor}>{s.label}</button>)}</div></div>
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 mb-6"><div className="bg-emerald-900/30 border border-emerald-700 rounded-xl p-2 sm:p-3 text-center"><div className="text-xl sm:text-2xl font-bold text-emerald-400">{st.h}</div><div className="text-[10px] sm:text-xs text-emerald-400">Hadir</div></div><div className="bg-yellow-900/30 border border-yellow-700 rounded-xl p-2 sm:p-3 text-center"><div className="text-xl sm:text-2xl font-bold text-yellow-400">{st.i}</div><div className="text-[10px] sm:text-xs text-yellow-400">Izin</div></div><div className="bg-primary-900/30 border border-primary-700 rounded-xl p-2 sm:p-3 text-center"><div className="text-xl sm:text-2xl font-bold text-primary-300">{st.s}</div><div className="text-[10px] sm:text-xs text-primary-300">Sakit</div></div><div className="bg-red-900/30 border border-red-700 rounded-xl p-2 sm:p-3 text-center"><div className="text-xl sm:text-2xl font-bold text-red-400">{st.a}</div><div className="text-[10px] sm:text-xs text-red-400">Alpha</div></div></div>
    <div className="overflow-x-auto"><table className="w-full"><thead><tr className="border-b-2 border-slate-700"><th className="text-left py-2 sm:py-3 px-2 sm:px-4 text-xs sm:text-sm font-semibold text-slate-400">No</th><th className="text-left py-2 sm:py-3 px-2 sm:px-4 text-xs sm:text-sm font-semibold text-slate-400">NIS</th><th className="text-left py-2 sm:py-3 px-2 sm:px-4 text-xs sm:text-sm font-semibold text-slate-400">Nama</th><th className="text-left py-2 sm:py-3 px-2 sm:px-4 text-xs sm:text-sm font-semibold text-slate-400">Status</th></tr></thead><tbody>{ent.map((e,i)=>(<tr key={e.studentId} className="border-b border-slate-700 hover:bg-slate-700/50"><td className="py-2 sm:py-3 px-2 sm:px-4 text-sm text-slate-400">{i+1}</td><td className="py-2 sm:py-3 px-2 sm:px-4 text-sm font-mono text-slate-400">{e.studentNis}</td><td className="py-2 sm:py-3 px-2 sm:px-4 text-sm font-medium text-slate-200">{e.studentName}</td><td className="py-2 sm:py-3 px-2 sm:px-4"><div className="flex flex-wrap gap-1 sm:gap-2">{SO.map(s=><button key={s.value} onClick={()=>cs(e.studentId,s.value)} className={"px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg text-[10px] sm:text-xs font-semibold border-2 "+(e.status===s.value?s.bgColor+" "+s.textColor+" "+s.borderColor+" shadow-lg shadow-black/20":"bg-slate-800 text-slate-500 border-slate-700")}>{s.label}</button>)}</div></td></tr>))}</tbody></table></div>
    <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 mt-6 pt-4 border-t border-slate-700"><div className="flex items-center gap-3">{ex&&<span className="bg-yellow-900/40 text-yellow-400 px-3 py-1.5 rounded-lg text-xs font-medium"><Clock size={14} className="inline mr-1"/>Sudah ada</span>}{sd&&<span className="bg-emerald-900/40 text-emerald-400 px-3 py-1.5 rounded-lg text-xs font-medium animate-pulse"><CheckCircle size={14} className="inline mr-1"/>Berhasil!</span>}</div><button onClick={sb} disabled={sv||!selT} className="bg-primary-600 text-white px-6 sm:px-8 py-3 rounded-xl font-semibold flex items-center justify-center gap-2 hover:bg-primary-700 disabled:opacity-50 transition-all shadow-lg shadow-black/20">{sv?<><div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"/> Menyimpan...</>:<><Save size={18}/> Simpan Absen</>}</button></div></div>)}
  </div>);
}
